...
async function simulateCandidateFlow(candidateIndex, testId, realQuestions, realCodingQuestions, token) {
  ...
  try {
    ...
    for (let i = 0; i < TOTAL_CANDIDATES; i += CONCURRENCY) {
      ...
      for (let j = 0; j < batchSize; j++) {
        batchPromises.push(simulateCandidateFlow(i + j + 1, testId, realQuestions, realCodingQuestions, validToken));
      }
