...
async function simulateActiveStudent(studentIndex, testId, realQuestions, realCodingQuestions, globalStats, token) {
...
async function runRealtime500ExamTest() {
  ...
  try {
    ...
    let userDoc = await mongoose.connection.db.collection('users').findOne({});
    if (!userDoc) {
      const dummyUser = { name: 'LoadTest User', email: 'loadtest@example.com', role: 'candidate', createdAt: new Date() };
      const insertRes = await mongoose.connection.db.collection('users').insertOne(dummyUser);
      userDoc = { _id: insertRes.insertedId, ...dummyUser };
    }
    const validToken = jwt.sign({ id: userDoc._id.toString(), role: userDoc.role || 'candidate' }, JWT_SECRET);
    ...
    for (let s = 1; s <= TOTAL_STUDENTS; s++) {
      studentPromises.push(simulateActiveStudent(s, testId, realQuestions, realCodingQuestions, globalStats, validToken));
    }
    ...
    console.log('\n⏳ HTTP Requests finished submitting! Waiting for BullMQ background workers to complete processing...');
    let queueDrainStartTime = Date.now();
    while (true) {
      try {
        ...
        // If queues endpoint unavailable, break after brief delay
        break;
      }
      await new Promise(r => setTimeout(r, 2000));
    }
